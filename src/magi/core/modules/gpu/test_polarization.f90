
program test_polarization
    use gpu_polarization
    implicit none
    complex(8), dimension(:,:), allocatable :: A, B, C, ref
    complex(8), dimension(:,:,:), allocatable :: G_lesser,G_greater,G_retarded,G_advanced
    real(8) :: tstart, tstop, elapsed_time
    real(8) :: gflops, err, L2
    real :: randnum
    integer, allocatable, dimension(:) :: jl,ki
    integer(8) :: devPtrA, devPtrB, devPtrC, devPtrGL,devPtrGG,devPtrGR,devPtrGA
    integer :: n=56
    integer :: m=140
    integer :: nen=6400
    integer :: nop=10
    ! integer :: size_of_real=8 !4->single precision; 8->double precision
    ! integer :: size_of_complex=16 !4->single precision; 8->double precision
    integer :: i,j,k,l,ie
  
    integer,dimension(8) :: values
    complex(8) :: a1,a2, Pijkl
    integer :: seed
    integer :: index
  
    real(8),parameter :: pi = 4.0*atan(1.0)
    real(8),parameter :: alpha = 0.99

    a1 = 1.0 - alpha
    a2 = alpha * 0.5

    call date_and_time(VALUES=values) !values(8) = milisecs of the second
    seed = values(8) !using value in milisecs as seeder
    print *, seed
    call srand(seed) !not a std implementation, but i like it better.
    
    allocate(G_lesser(m,m,nen))
    allocate(G_greater(m,m,nen))
    allocate(G_retarded(m,m,nen))
    ! allocate(G_advanced(m,m,nen))

    allocate(jl(n))
    allocate(ki(n))
    allocate(C(m,m))
    allocate(ref(m,m))
    
    !solution matrix
    C = dcmplx(0.0,0.0)
    !reference matrix
    ref = dcmplx(0.0,0.0)
    !initiating matrix value
    
    do j=1,m
        do i=1,nen
            call random_number(randnum)
            G_greater(j,:,i) = dcmplx(randnum,0.0)
            call random_number(randnum)
            G_lesser(j,:,i) = dcmplx(randnum,0.0)            
            call random_number(randnum)
            g_retarded(j,:,i) = dcmplx(randnum,0.0)
        enddo
    enddo

    ! do j = 1, n
    !     jl(j) = (l-1)*m + j ! j=j l=1
    !     ki(j) = (i-1)*m + k ! k=j i=1 
    ! enddo 

    i=1
    j=1
    k=1
    l=1

    call cpu_time(tstart)
    call gpu_polarization_prepare(&
        nen=nen,nm=m,&
        G_lesser=g_lesser,G_greater=g_greater,G_retarded=g_retarded,&
        devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA)        
    call cpu_time(tstop)
    elapsed_time = tstop - tstart !in seconds
    write(*,20) 'Elapsed time : ',elapsed_time, 'secs'
    call omp_set_num_threads(1)
    call cpu_time(tstart)
    !$omp parallel default(shared) private(i,j,k,l,Pijkl,devPtrA,devPtrB,devPtrC)
    call gpu_polarization_calc_ijkl(&
                                    a1=a1,a2=a2,nop=nop,nen=nen,nm=m,i=1,j=1,k=1,l=1,&
                                    devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA,&
                                    devPtrA=devPtrA,devPtrB=devPtrB,devPtrC=devPtrC,&
                                    P_ijkl = Pijkl, malloc_gpu=.true.)
    !$omp do   
    do j=1, m
        print *,j
        do i=1,n
            do k=1,n
                do l=1,n
                    call gpu_polarization_calc_ijkl(&
                        a1=a1,a2=a2,nop=nop,nen=nen,nm=m,i=i,j=j,k=k,l=l,&
                        devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA,&
                        devPtrA=devPtrA,devPtrB=devPtrB,devPtrC=devPtrC,&
                        P_ijkl=Pijkl,malloc_gpu=.false.)
                    C(j,i)=C(j,i)+Pijkl
                enddo
            enddo
        enddo
    enddo 
    !$omp end do        
    call cublas_free(devPtrA)
    call cublas_free(devPtrB)
    call cublas_free(devPtrC)
    !$omp end parallel    

    call cpu_time(tstop)
    elapsed_time = tstop - tstart !in seconds
    write(*,20) 'Elapsed time : ',elapsed_time, 'secs'

    call gpu_polarization_finalize( &
        devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA)

    ! call cpu_time(tstart)
    ! call gpu_polarization(a1=a1,a2=a2,nop=nop,nen=nen,nm=m,num_jl=n,jl=jl,num_ki=n,ki=ki,copy_to_gpu=.true.,malloc_gpu=.true.,&
    !     G_lesser=G_lesser,G_greater=G_greater,G_retarded=G_retarded,G_advanced=G_advanced,&
    !     devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA,&
    !     devPtrA=devPtrA,devPtrB=devPtrB,devPtrC=devPtrC,&
    !     partial_P=C)
    ! call cpu_time(tstop)

    ! elapsed_time = tstop - tstart !in seconds
    ! write(*,20) 'Elapsed time : ',elapsed_time, 'secs'

    ! ! print *,C(1,1)
    
    20 format(A15,2X,1F0.8,2X,A4)  

    ! do j=1,20
    !     ki = ki + 1
    !     if (ki(1) > m*m) then 
    !         ki = 1
    !     endif

    !     call cpu_time(tstart)

    !     call gpu_polarization(a1=a1,a2=a2,nop=nop,nen=nen,nm=m,num_jl=n,jl=jl,num_ki=n,ki=ki,copy_to_gpu=.false.,malloc_gpu=.false.,&
    !         G_lesser=G_lesser,G_greater=G_greater,G_retarded=G_retarded,G_advanced=G_advanced,&
    !         devPtrGL=devPtrGL,devPtrGG=devPtrGG,devPtrGR=devPtrGR,devPtrGA=devPtrGA,&
    !         devPtrA=devPtrA,devPtrB=devPtrB,devPtrC=devPtrC,&
    !         partial_P=C)

    !     call cpu_time(tstop)

    !     elapsed_time = tstop - tstart !in seconds
    !     ! print *,C(1,1)
    !     write(*,20) 'Elapsed time : ',elapsed_time, 'secs'
    ! enddo
    
    ! print *,C(1,1)
    
    ! !Free GPU memory
    ! call cublas_free(devPtrGR)
    ! call cublas_free(devPtrGL)
    ! call cublas_free(devPtrGG)
    ! call cublas_free(devPtrGA)
    ! call cublas_free(devPtrA)
    ! call cublas_free(devPtrB)
    ! call cublas_free(devPtrC)

    !reference from CPU
    call cpu_time(tstart)
    call omp_set_num_threads(20)
    !$omp parallel default(shared) private(i,j,k,l)    
    !$omp do  
    do j=1,m
        do i=1,n
            do k=1,n
                do l=1,n
                    ref(j,i) = ref(j,i) + a2* sum( G_greater(j,l,1+nop:nen) * G_lesser(k,i,1:nen-nop) ) &
                        - a2* sum( G_lesser(j,l,1+nop:nen) * G_greater(k,i,1:nen-nop) ) &
                        + a1* sum( G_lesser(j,l,1+nop:nen) * conjg( G_retarded(i,k,1:nen-nop) ) ) &
                        + a1* sum( G_retarded(j,l,1+nop:nen) * G_lesser(k,i,1:nen-nop) ) 
                enddo
            enddo
        enddo
    enddo 
    !$omp end do        
    !$omp end parallel   
    call cpu_time(tstop)
    
    elapsed_time = tstop - tstart !in seconds
    write(*,20) 'CPU Elapsed t : ',elapsed_time, 'secs'
    
    do i=1,n
        print *,C(i,i),ref(i,i)
    enddo

    index = m
    err = 0.0
    !show result
    do j = 1, index
       do i = 1, index
          err = err + abs( ref(i,j)- C(i,j) )**2
       end do
    end do
    L2 = sqrt( err / ( dble(index)*dble(index) ) )
    write(*,30) 'L2-residual :', L2
  30 format(A15,2X,1E18.8)
    
    
  end program test_polarization
  
