...
module eph_rgf 
    ...
    subroutine solve_eph_rgf_3d(nx,mm,nm, nen, energies, nphiy, nphiz, niter, scba_tol, &
        ...
        allocate(sigma_lesser_ph(mm,nx,nen), source=czero)
        allocate(sigma_r_ph(mm,nx,nen), source=czero)
        allocate(sigma_lesser_ph_new(mm,nx,nen), source=czero)
        allocate(sigma_greater_ph_new(mm,nx,nen), source=czero)
        allocate(sigma_r_ph_new(mm,nx,nen), source=czero)
        ! full self-energies
        !allocate(sigma_lesser_ph_full(mm,mm,nx), source=czero)
        !allocate(sigma_r_ph_full(mm,mm,nx), source=czero)
        mul(:,:)=mus
        mur(:,:)=mud
        TEMPl(:,:)=temps
        TEMPr(:,:)=tempd
