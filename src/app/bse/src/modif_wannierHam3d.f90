...
integer :: n_atom
real(8), allocatable :: atoms_pos_cart(:,:) ! atom positions cart
real(8), allocatable :: atoms_pos_frac(:,:) ! atom positions frac
character(len=2),allocatable:: name_atoms(:) ! chemical symbol of atoms
REAL(8), DIMENSION(3) :: alpha,beta,gamm,xhat,yhat,zhat,b1,b2,b3
REAL(8) :: Lx, Ly,Lz,cell(3,3) ! in Ang
REAL(8) :: CBM, VBM, Eg ! CB minimum and VB maximum
REAL(8) :: kt_CBM, kt_VBM ! transverse ky value of CBM and VBM location
REAL(8) :: ktz_CBM, ktz_VBM ! transverse kz value of CBM and VBM location
...
public :: NB, Lx, Ly,Lz, Nvb, VBM, CBM, kt_CBM, kt_VBM, ktz_CBM, ktz_VBM, Eg, spin_deg,xhat,yhat,zhat,cell,n_atom,name_atoms,atoms_pos_cart,atoms_pos_frac
public :: eig,cross,eigv,b1,b2,b3,norm,wannier_center,alpha,beta,gamm, invert
public :: w90_MAT_DEF_full_device, w90_bare_coulomb_full_device
public :: w90_mat_def_kv
...
if (rank == 0) then    
    ...
    b3 = cross(alpha,beta)/dot_product(beta,cross(gamm,alpha)) 
    ...
    nkx=min(floor((1.0d0/Lx)/0.0025d0)+1 , 25)
    nky=min(floor((1.0d0/Ly)/0.0025d0)+1 , 25)    
    nkz=min(floor((1.0d0/Lz)/0.0025d0)+1 , 25)
    if (nx==1) nkx=1
    if (ny==1) nky=1
    if (nz==1) nkz=1
    if (ny .eq. 1) then
    ...
    kt_vbm = (mod(ind(nvb),nkx*nky)/nkx)*dky - pi / Ly ! ky transverse corresponding to VBMax      
    ktz_vbm = ind(nvb)/nkx/nky*dkz - pi / Lz ! kz transverse corresponding to VBMax      
    ind = minloc(energ,2)
    CBM = energ(nvb+1,ind(nvb+1))
    kt_cbm = (mod(ind(nvb+1),nkx*nky)/nkx)*dky - pi / Ly ! ky transverse corresponding to CBM
    ktz_cbm = ind(nvb+1)/nkx/nky*dkz - pi / Lz ! kz transverse corresponding to CBM
...
if (rank == 0) then    
    ...
    print '(2f8.3)', ktz_cbm/(2.0*pi/Ly), ktz_vbm/(2.0*pi/Ly)
    ...
    read(fid,*) comment    
    read(fid,*) comment    
    if (comment(1:4) == 'cart') then 
        read(fid,*) n_atom
        allocate(atoms_pos_cart(3,n_atom))
        allocate(atoms_pos_frac(3,n_atom))
        allocate(name_atoms(n_atom))
        atoms_pos_frac = 0.0d0
        do i=1,n_atom
          read(fid,*) name_atoms(i), atoms_pos_cart(:,i)
        enddo  
    else
        read(fid,*) n_atom
        allocate(atoms_pos_cart(3,n_atom))
        atoms_pos_cart = 0.0d0
        allocate(atoms_pos_frac(3,n_atom))
        allocate(name_atoms(n_atom))
        do i=1,n_atom
          read(fid,*) name_atoms(i), atoms_pos_frac(:,i)
          do j=1,3
            atoms_pos_cart(:,i) = atoms_pos_cart(:,i)+ atoms_pos_frac(j,i) * cell(:,j)
          enddo
        end do        
    endif
    deallocate(ham)
...
SUBROUTINE w90_momentum_full_device(Ham,ky,kz,length,method,NS)
implicit none
integer, intent(in) :: length
integer, intent(in), optional :: NS
real(8), intent(in) :: ky,kz
...
pre_fact= 1.0d-10 * dcmplx(0.0d0,1.0d0) * m0/hbar * c0  ! multiply light-speed so (c0*pmn) in same unit as H [eV]
select case (method)
  case ('approx')
  ! use wannier centers, point-like orbitals
    ...
    pmn=pmn*pre_fact
  case ('exact')
  ! use position operator : i m_0/hbar sum_{R'l} H_{nl}(R-R') r_{lm}(R') - r_{nl}(R-R') H_{lm}(R')
    pmn = 0.0d0
    do io=1,NB
      do jo=1,NB
        do ix=xmin,xmax
...
!!! construct the fully periodic Hamiltonian matrix
SUBROUTINE w90_MAT_DEF_kv(Hii,kv)
  ...
  integer :: i,j,k
    ...
    do i = xmin,xmax           
      do j = ymin,ymax
        do k = zmin,zmax
          r = i*alpha + j*beta + k*gamm                                   
          Hii(:,:) = Hii(:,:) + Hr(:,:,i-xmin+1,j-ymin+1,k-zmin+1) &
          &* exp(-z1j * dot_product(r,kv))                                  
        enddo
      end do
        !!! Hii = (Hii + transpose(conjg(Hii))) / 2.0d0    
    end do   
  END SUBROUTINE w90_MAT_DEF_kv
