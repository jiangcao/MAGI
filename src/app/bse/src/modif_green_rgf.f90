...
public :: green_rgf_solve_gw_ephoton_3d_ijs
...
subroutine green_rgf_solve_gw_ephoton_3d(alpha_mix,niter,NB,NS,nm,nx,nky,nkz,ndiag,&
  Lx,nen,en,temp,mu,Hii,H1i,Vii,V1i,spindeg,Pii,P1i,polarization,intensity,hw,labs)
  ...
  ! -------- local variables
  real(8), parameter :: pre_fact=((hbar/m0_kg)**2)/(2.0d0*epsilon0*c0**3) 
  ...
  integer::ie,iter,i,j,ix,nopmax,nop,nopphot,iop,l,h,io
...
!  allocate(W_retarded_i1(nm,nm,nx,nen,nk))  
  ...
  do iter=0,niter
    ...
    do ik=1,nk
      ...
      !$omp end do 
      !$omp end parallel
      call calc_block_current(Hii(:,:,:,ik),G_lesser(:,:,:,:,ik),cur(:,:,:,:,ik),nen,en,spindeg,nb,ns,nm,nx,&
                              tot_cur(:,:,:,ik),tot_ecur(:,:,:,ik),jdens(:,:,:,:,ik))      
    enddo
    tr =tr *(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg)/dble(nk)
    tre=tre*(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg)/dble(nk)
    !
    ...
    write(101,'(I4,2E16.6)') iter, sum(tre), sum(tr) 
    close(101)
...
!    P_retarded_1i = dcmplx(0.0d0,0.0d0)  
    ...
    do iqy=1,nky        
      do iqz=1,nkz
        iq=iqz + (iqy-1)*nkz
        do ix=1,nx
          !$omp parallel default(none) private(l,h,i,j,ikz,ikzd,iqz,iky,ikyd,iqy,ik,ikd) shared(ix,ndiag,P_lesser,P_greater,P_retarded,nen,En,nm,G_lesser,G_greater,G_r,iq,nx,nkz,nky,nk,cmethod)    
          !$omp do              
          do i=1,nm      
            l=max(i-ndiag,1)
            h=min(nm,i+ndiag)   
            do j=l,h
              do iky=1,nky
                do ikz=1,nkz              
                  ...
                  P_lesser(i,j,ix,:,iq) = P_lesser(i,j,ix,:,iq) +  corr1d(nen, G_lesser(i,j,ix,:,ik), G_greater(j,i,ix,:,ikd),method=cmethod)
                  P_greater(i,j,ix,:,iq) = P_greater(i,j,ix,:,iq) + corr1d(nen, G_greater(i,j,ix,:,ik), G_lesser(j,i,ix,:,ikd),method=cmethod)        
                  P_retarded(i,j,ix,:,iq) = P_retarded(i,j,ix,:,iq) &
                                              + corr1d(nen, G_lesser(i,j,ix,:,ik), conjg(G_r(i,j,ix,:,ikd)),method=cmethod)  & 
                                              + corr1d(nen, G_r(i,j,ix,:,ik), G_lesser(j,i,ix,:,ikd), method=cmethod)        
      ...
      do ikz=1,nkz                  
        ...
        do iqy=1,nky
          do iqz=1,nkz              
            ...
            do ix=1,nx
              !$omp parallel default(none) private(l,h,i,j) shared(ik,ikd,iq,ix,ndiag,nopmax,w_lesser,w_greater,w_retarded,sigma_lesser_new,sigma_greater_new,sigma_r_new,nen,En,nm,G_lesser,G_greater,G_r,nx,nkz,nky,nk,cmethod)    
              !$omp do    
              do i=1,nm                    
                l=max(i-ndiag,1)
                h=min(nm,i+ndiag)                                                
                do j=l,h
                  Sigma_lesser_new(i,j,ix,:,ik)=Sigma_lesser_new(i,j,ix,:,ik) &
                                    + conv1d(nen, G_lesser(i,j,ix,:,ikd), W_lesser(i,j,ix,:,iq), method=cmethod)
                  Sigma_greater_new(i,j,ix,:,ik)=Sigma_greater_new(i,j,ix,:,ik) &
                                    + conv1d(nen, G_greater(i,j,ix,:,ikd), W_greater(i,j,ix,:,iq), method=cmethod)
                  Sigma_r_new(i,j,ix,:,ik)=Sigma_r_new(i,j,ix,:,ik) &
                                    + conv1d(nen, G_lesser(i,j,ix,:,ikd), W_retarded(i,j,ix,:,iq), method=cmethod) &
                                    + conv1d(nen, G_r(i,j,ix,:,ikd), W_lesser(i,j,ix,:,iq), method=cmethod) &
                                    + conv1d(nen, G_r(i,j,ix,:,ikd), W_retarded(i,j,ix,:,iq), method=cmethod)    
                enddo                                 
              enddo
              !$omp end do
              !$omp end parallel
...
subroutine green_rgf_solve_gw_ephoton_3d_ijs(alpha_mix,niter,NB,NS,nm,nx,nky,nkz,ndiag,Lx,nen,en,temp,mu,Hii,H1i,Vii,V1i,spindeg,Pii,P1i,polarization,intensity,hw,labs, comm_size, comm_rank, local_NE, first_local_energy)
  ...
  do iter=0,niter
    ...
    if (comm_rank == 0) then
      open(unit=101,file='Id_iteration.dat',status='unknown',position='append')
      write(101,'(I4,2E16.6)') iter, global_sum_tr*(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg)/dble(nk), &
                                     global_sum_tre*(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg)/dble(nk)
      close(101)
      call write_current_summed_over_k('gw_I',iter,tot_cur,nx*ns,NB,Lx,nk)
      call write_current_summed_over_k('gw_EI',iter,tot_ecur,nx*ns,NB,Lx,nk)      
    endif
    ...
    do iq=1,nky*nkz
      ...
      do nop = max(-nopmax + nen/2, first_local_energy), min(nopmax + nen/2, first_local_energy + local_NE - 1)
        ...
        call green_rgf_calc_w( 0, nm, nx, Vii(:,:,:,iq), V1i(:,:,:,iq), &
          p_lesser_by_energies(:,:,:,ie,iq), p_greater_by_energies(:,:,:,ie,iq), p_retarded_by_energies(:,:,:,ie,iq), &
          w_lesser_by_energies(:,:,:,ie,iq), w_greater_by_energies(:,:,:,ie,iq), w_retarded_by_energies(:,:,:,ie,iq))  
      enddo
      !$omp end do 
    ...
    !
    !!!!! calculate collision integral of GW
    !
...
subroutine green_rgf_solve_gw_1d(alpha_mix,niter,NB,NS,nm,nx,ndiag,Lx,nen,en,temp,mu,Hii,H1i,Vii,V1i,spindeg)
  ...
  do iter=0,niter
    ...
    write(101,'(I4,2E16.6)') iter, sum(tre)*(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg), sum(tr)*(En(2)-En(1))*e_charge/twopi/hbar*e_charge*dble(spindeg)
    close(101)
    g_r = dcmplx( 0.0d0*dble(g_r), aimag(g_r))
    g_lesser = dcmplx( 0.0d0*dble(g_lesser), aimag(g_lesser))
    g_greater = dcmplx( 0.0d0*dble(g_greater), aimag(g_greater))
...
integer::ie,ix  
  ...
  do ix=1,nx
    !$omp parallel default(shared) private(ie,B) 
    allocate(B(nm,nm))
    !$omp do 
    do ie=1,nen    
    ...
    !$omp end parallel
  enddo
  I(:,:,:)=I(:,:,:)*dble(en(2)-en(1))/twopi*spindeg
...
subroutine calc_n_electron(G_lesser,G_greater,nen,nk,E,nm,nelec,pelec,midgap,bandgap,CBM,VBM)
  complex(8), intent(in) :: G_lesser(nm,nm,nen,nk)
  complex(8), intent(in) :: G_greater(nm,nm,nen,nk)
  real(8), intent(in)    :: E(nen),midgap,bandgap
  integer, intent(in)    :: nm,nen,nk
  real(8), intent(out)   :: nelec,pelec,CBM,VBM
  real(8)::dE, trn,trp
  integer::ie,ik,j
  nelec=0.0d0
  pelec=0.0d0
  ...
  do ie=1,nen
    ...
    do ik=1,nk    
      do j=1,nm
        trn=trn+aimag(G_lesser(j,j,ie,ik))
        trp=trp+aimag(G_greater(j,j,ie,ik))
      enddo
    enddo
    trn=trn*dE/dble(nk)
    trp=-trp*dE/dble(nk)
    ! determine new conduction/valence band min/max
    if ( abs(E(ie)-midgap) < (bandgap/2.0d0+1.0d0)) then
      if ((trn+trp) < 1e-2) then
        if  (E(ie) > CBM) then
          CBM = E(ie)
        endif
        if  (E(ie) < VBM) then
    ...
    ! determine electron/hole density  
    if (E(ie)>midgap)then
      nelec=nelec+trn
    else
      pelec=pelec+trp
