...
! --------
complex(8)::A(nm,nm),B(nm,nm),C(nm,nm),D(nm,nm),Wn(nm,nm),Wp(nm,nm),sig(nm,nm),AL(nm,nm),BL(nm,nm),BG(nm,nm)
complex(8),allocatable,dimension(:,:,:)::S,M,LL,LG,VV,M1i,Mi1,S1i,Si1,WRi1_,WLi1_,WGi1_
...
do i=1,nx
  ...
  call zgemm('n','c',nm,nm,nm,cone,A,nm,Vii(:,:,i),nm,cone,LL(:,:,i),nm)
  !
  call zgemm('c','n',nm,nm,nm,cone,V1i(:,:,i),nm,PL(:,:,min(i+1,nx)),nm,czero,A,nm)
  call zgemm('n','c',nm,nm,nm,cone,A,nm,V1i(:,:,i),nm,cone,LL(:,:,i),nm)
  !
  call zgemm('n','n',nm,nm,nm,cone,V1i(:,:,max(i-1,1)),nm,PL(:,:,max(i-1,1)),nm,czero,A,nm)
  call zgemm('n','n',nm,nm,nm,cone,A,nm,V1i(:,:,max(i-1,1)),nm,cone,LL(:,:,i),nm)
  !
...
do i=1,nx
  ...
  call zgemm('n','c',nm,nm,nm,cone,A,nm,Vii(:,:,i),nm,cone,LG(:,:,i),nm)
  !
  call zgemm('c','n',nm,nm,nm,cone,V1i(:,:,i),nm,PG(:,:,min(i+1,nx)),nm,czero,A,nm)
  call zgemm('n','c',nm,nm,nm,cone,A,nm,V1i(:,:,i),nm,cone,LG(:,:,i),nm)
  !
  call zgemm('n','n',nm,nm,nm,cone,V1i(:,:,max(i-1,1)),nm,PG(:,:,max(i-1,1)),nm,czero,A,nm)
  call zgemm('n','n',nm,nm,nm,cone,A,nm,V1i(:,:,max(i-1,1)),nm,cone,LG(:,:,i),nm)
  !
...
do ix=nx-1,1,-1
  ...
  call zgemm('n','n',nm,nm,nm,cone,B,nm,M1i(:,:,ix),nm,czero,C,nm)
  A = M(:,:,ix) - C
  call invert(A,nm) ! A -> xr
  Xlr(:,:,ix)=A     
  call zgemm('n','n',nm,nm,nm,cone,B,nm,V1i(:,:,ix),nm,czero,C,nm) ! C -> MxR Vn+1,n
  D=VV(:,:,nx) - C  ! D -> Vn,n - MxR Vn+1,n
...
call zgemm('n','t',nm,nm,nm,cone,B,nm,xlr(:,:,2),nm,czero,C,nm) 
WRi1_(:,:,1)=transpose(V1i(:,:,1)) - C
xR(:,:,1)=xlr(:,:,1)
WL(:,:,1)=Wln(:,:,1)
WG(:,:,1)=Wlp(:,:,1)
do ix=2,nx
  call zgemm('n','n',nm,nm,nm,cone,xlr(:,:,ix),nm,M1i(:,:,ix-1),nm,czero,B,nm) ! B -> xlr1 * M10
  call zgemm('n','n',nm,nm,nm,cone,B,nm,WRi1_(:,:,ix-1),nm,czero,C,nm) 
  WR(:,:,ix)=Wlr(:,:,ix)  - C  
  ...
  call zgemm('n','n',nm,nm,nm,cone,B,nm,xR(:,:,ix-1),nm,czero,D,nm)   
  ...
  call zgemm('n','n',nm,nm,nm,cone,A,nm,C,nm,czero,D,nm)    ! D -> A M01 xlr1
  WL(:,:,ix)=wln(:,:,ix)  + D + (BL-transpose(conjg(BL)))
  ! BG=xlr M10 XR M01 wlp  
  call zgemm('n','n',nm,nm,nm,cone,B,nm,xR(:,:,ix-1),nm,czero,D,nm)   
  call zgemm('n','n',nm,nm,nm,cone,D,nm,Mi1(:,:,ix-1),nm,czero,A,nm)  
  call zgemm('n','n',nm,nm,nm,cone,A,nm,wlp(:,:,ix),nm,czero,BG,nm)   
  !
  call zgemm('n','n',nm,nm,nm,cone,B,nm,WG(:,:,ix-1),nm,czero,A,nm) 
  call zgemm('n','n',nm,nm,nm,cone,A,nm,C,nm,czero,D,nm)   
  WG(:,:,ix)=wlp(:,:,ix)  + D + (BG-transpose(conjg(BG)))
  !
  if (ix<nx) then
    call zgemm('n','t',nm,nm,nm,cone,WR(:,:,ix),nm,M1i(:,:,ix),nm,czero,B,nm)
    call zgemm('n','t',nm,nm,nm,cone,B,nm,xlr(:,:,ix+1),nm,czero,C,nm) 
    WRi1_(:,:,ix)=transpose(V1i(:,:,ix)) - C
  endif
  !     
enddo
!
