...
use setup_mod
use parameters_mod
USE wannierHam3d, only : NB, w90_load_from_file, w90_free_memory,Ly,Lz,CBM,VBM,eig,w90_MAT_DEF_full_device, invert, Lx, w90_bare_coulomb_full_device,kt_CBM,spin_deg,Eg,w90_MAT_DEF,w90_bare_coulomb_blocks,w90_momentum_blocks,w90_momentum_full_device
use green, only : green_calc_g, green_solve_gw_3D,green_solve_gw_1D,green_solve_gw_ephoton_1D,identity,green_solve_gw_1D_memsaving,green_solve_gw_ephoton_3D
use green_rgf, only : green_rgf_solve_gw_1d,green_RGF_CMS, green_rgf_solve_gw_ephoton_3d_ijs,green_rgf_solve_gw_ephoton_3d

implicit none

integer :: nm, ie, ne, width,i,j,k,axis,num_B,ib,num_vac,iky,ik
...
complex(8), allocatable :: Pmn(:,:,:,:), eye(:,:)

logical ::  lqdot
real(8), allocatable :: pot(:)
integer, allocatable :: cell_index(:,:)
integer :: nm_dev, iter,ikz,nk
real(8) ::  dkz,kz,encut(2),dky
real(8) :: midgap(2)
real(8) :: start, finish
...
if (ltrans) then    
  if (.not.lrgf) then    
      ...
      ! momentum operator   
      allocate(Pmn(nb*length,nb*length,3,nky*nkz))      
      !
      ...
      do iky=1,nky
        ...
        do ikz=1,nkz
          ...
          ! construct contacts and coupling
          if (lflatband) then
            allocate(eye(nb*ns,nb*ns))
            call identity(eye,nb*ns)
            T(1:nb*ns,1:nb*ns,1,ik) = eye(:,:) * sqrt(ita)
            T(1:nb*ns,nb*(length-ns)+1:nb*length,2,ik) = eye(:,:) * sqrt(ita)
            deallocate(eye)
            ...
            do j=1,nb*NS
                do i=1,nkx
                    write(11,'(4E18.8)') ky, kz, phix(i)/Lx, ek(j,i)
                enddo
                write(11,*)
            enddo            
            deallocate(ek,phix)        
      ...
      do iky=1,nky
        ...
        do ikz=1,nkz
          ...
          ! momentum matrix
          call w90_momentum_full_device(Pmn(:,:,:,ik),ky,kz,length,'approx')
        enddo
      enddo
      open(unit=11,file='V.dat',status='unknown')
      do i=1, size(V,1)
          do j=1, size(V,2)
              write(11,'(2I6,2E15.4)') i,j, dble(V(i,j,nky*nkz/2+1)), aimag(V(i,j,nky*nkz/2+1))
          end do
          write(11,*)
      end do
      close(11)
      ...
      do i=1, size(Ham,1)
          do j=1, size(Ham,2)
              write(11,'(2I6,2E15.4)') i,j, dble(Ham(i,j,nky*nkz/2+1)), aimag(Ham(i,j,nky*nkz/2+1))
          end do
          write(11,*)
      end do
      close(11)
      
      open(unit=11,file='Pmn_x.dat',status='unknown')
      do i=1, size(Pmn,1)
          do j=1, size(Pmn,2)
              write(11,'(2I6,2E15.4)') i,j, dble(Pmn(i,j,1,nky*nkz/2+1)), aimag(Pmn(i,j,1,nky*nkz/2+1))
          end do
          write(11,*)
      end do
      close(11)
      
      open(unit=11,file='Pmn_y.dat',status='unknown')
      do i=1, size(Pmn,1)
          do j=1, size(Pmn,2)
              write(11,'(2I6,2E15.4)') i,j, dble(Pmn(i,j,1,nky*nkz/2+1)), aimag(Pmn(i,j,2,nky*nkz/2+1))
          end do
          write(11,*)
      end do
      close(11)
      
      open(unit=11,file='Pmn_z.dat',status='unknown')
      do i=1, size(Pmn,1)
          do j=1, size(Pmn,2)
              write(11,'(2I6,2E15.4)') i,j, dble(Pmn(i,j,1,nky*nkz/2+1)), aimag(Pmn(i,j,3,nky*nkz/2+1))
          end do
          write(11,*)
      end do
      close(11)
      ...
      encut=(/E_cutoff,E_cutoff/)

      start = MPI_Wtime()
      
      if (lkz) then
        if (lephot) then
          call green_solve_gw_ephoton_3D(niter,nm_dev,Lx,length,dble(spin_deg),temps,tempd,mus,mud,&
            ...
            Sig_retarded_new,Sig_lesser_new,Sig_greater_new,encut,Eg,lvertex,lbse,lflatband,ndiag)
        else
        ...
        if (lephot) then
          call green_solve_gw_ephoton_1D(niter,nm_dev,Lx,length,dble(spin_deg),temps,tempd,mus,mud,midgap,&
            ...
            Pmn(:,:,:,1),polarization,intensity,hw,labs,&
            ...
            Sig_retarded_new(:,:,:,1),Sig_lesser_new(:,:,:,1),Sig_greater_new(:,:,:,1),ldiag,encut,Eg,lvertex,lbse,ndiag=ndiag)
        else        
          ...
          call green_solve_gw_1D_memsaving(niter,nm_dev,Lx,length,dble(spin_deg),temps,tempd,mus,mud,midgap,&
            alpha_mix,nen,En,nb,ns,Ham(:,:,1),H00ld(:,:,:,1),H10ld(:,:,:,1),T(:,:,:,1),V(:,:,1),&
            G_retarded(:,:,:,1),G_lesser(:,:,:,1),G_greater(:,:,:,1),&
            Sig_retarded(:,:,:,1),Sig_lesser(:,:,:,1),Sig_greater(:,:,:,1),&
            Sig_retarded_new(:,:,:,1),Sig_lesser_new(:,:,:,1),Sig_greater_new(:,:,:,1),&
            ldiag,ndiag,encut,Eg,writeGF=.false.,lvertex=lvertex,lbse=lbse)
...
!    allocate(V(nm*length,nm*length,nky*nkz))   
    ...
    do iky=1,nky
      ...
      do ikz=1,nkz
        kz=-pi/Lz + dble(ikz)*dkz + kz_shift*2.0d0*pi/Lz
        ik=ikz+(iky-1)*nkz
        ...
        if (comm_rank == 0) then
          ...
          do i=1,nm
            do j=1,nm
              write(10,'(2I6,2E15.6)') i,j,dble(Hii(i,j,1,ik)),aimag(Hii(i,j,1,ik))
              write(11,'(2I6,2E15.6)') i,j,dble(H1i(i,j,1,ik)),aimag(H1i(i,j,1,ik))
              write(20,'(2I6,2E15.6)') i,j,dble(Vii(i,j,1,ik)),aimag(Vii(i,j,1,ik))
              write(21,'(2I6,2E15.6)') i,j,dble(V1i(i,j,1,ik)),aimag(V1i(i,j,1,ik))
            enddo
          enddo
          write(10,*)
          write(11,*)
    ...
    if (lephot) then
        if (comm_size > 1) then
          call green_rgf_solve_gw_ephoton_3d_ijs(alpha_mix,niter,NB,NS,nm,length,nky,nkz,ndiag,Lx,nen,en,(/temps,tempd/),(/mus,mud/),Hii,H1i,Vii,V1i,dble(spin_deg),Pii,P1i,polarization,intensity,hw,labs, comm_size, comm_rank, local_NE, first_local_energy)
        else
          call green_rgf_solve_gw_ephoton_3d(alpha_mix,niter,NB,NS,nm,length,nky,nkz,ndiag,&
                        Lx,nen,en,(/temps,tempd/),(/mus,mud/),Hii,H1i,Vii,V1i,dble(spin_deg),Pii,P1i,polarization,intensity,hw,labs)
        endif
    else
      
        call green_rgf_solve_gw_ephoton_3d_ijs(alpha_mix,niter,NB,NS,nm,length,nky,nkz,ndiag,Lx,nen,en,(/temps,tempd/),(/mus,mud/),Hii,H1i,Vii,V1i,dble(spin_deg),Pii,P1i,polarization,0.0d0,0.0d0,.false., comm_size, comm_rank, local_NE, first_local_energy)        
