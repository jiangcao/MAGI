...
folder_save='/usr/scratch/mont-fort1/sem24f6/MAGI/f2py/QE_local-tests/Save_hetero3/'
label='hetero0'
# %%
...
def make_dk_list(k_list):
    list=[]
    for k in range(len(k_list)-1):
        dk=(k_list[k+1]-k_list[k])/(nkz-1)
        list.append(dk)
    dk=(k_list[0]-k_list[-1])/(nkz-1)
    list.append(dk)
    return list

k_list=[K,Gamma,M]

dk_list=make_dk_list(k_list)
l=len(dk_list)
...
for j in range(l):
    for ikz in range(nkz):
        ...
        G_retarded_balist,G_lesser_balist,G_greater_balist,cur_balist,te_balist[ik] = gf_dense.calc_gf(ne=nen,e=energies,num_lead=2,nm_dev=nb*length,nm_lead=dim_lead,max_nm_lead=nb*ns,
...
while delta> epsilon and niter<30:
    for iky in range(nky):
        for ikz in range(nkz):
            ...
            G_retarded[ik],G_lesser[ik],G_greater[ik],cur[ik],te[ik] = gf_dense.calc_gf(ne=nen,e=energies,num_lead=2,nm_dev=nb*length,nm_lead=dim_lead,max_nm_lead=nb*ns,
    ...
    sig_l_photon,sig_g_photon=gf_dense.selfenergy_eph_mono(nm=nb*length,nen=nen,en=energies,nop=nop,nky=nky,nkz=nkz,nqy=1,nqz=1,ik_start=1,ik_end=nky*nkz,iq_in=1,m=M_mat,g_lesser=np.transpose(G_lesser,(1,2,3,0)),g_greater=np.transpose(G_greater,(1,2,3,0)),
    ...
    #calculation of acoustic phonons SE

    # for iq in range(nk):
    #     for ie in range (nen):
    #         for ind in range(Number_acoust_mode):
    #             n=bedistribution(sc.e*dE*nac[ind,iq],temp[0])
    #             if ie > nac[ind,iq]:
    #                 intermed=int(ie-nac[ind,iq])
    #                 sig_l_int[...,ie]+=Dac2[ind]*(n*G_lesser[iq,:,:,intermed])
    #                 sig_g_int[...,ie]+=Dac2[ind]*(n*G_greater[iq,:,:,intermed])
    #             if ie< nen - nac[ind,iq]:
                    
    #                 intermed=int(ie+nac[ind,iq])
    #                 sig_l_int[...,ie]+=Dac2[ind]*((n+1)*G_lesser[iq,:,:,intermed])
    #                 sig_g_int[...,ie]+=Dac2[ind]*((n+1)*G_greater[iq,:,:,intermed])


    # print(sig_l_int)

    # for ind in range(len(Dac2)):
    #     sig_l_int+=np.sum(np.transpose(G_lesser_int,(1,2,0,3))[...,ind_ac_sup[ind]],axis=2)[...,:-1]*Dac2[ind]
    #     sig_g_int+=np.sum(np.transpose(G_greater_int,(1,2,0,3))[...,ind_ac_sup[ind]],axis=2)[...,:-1]*Dac2[ind]
        
    # sig_l_int+=Number_acoust_mode*sum_g_less[...,:-1]*Dac2
    # sig_g_int+=Number_acoust_mode*sum_g_great[...,:-1]*Dac2
...
#ind_final=ind_final%nkz*nky+ind_final//nkz

# # %%
# fig, ax = plt.subplots()

# XY=np.array([ind2coord(N=nkz,ik=ik,starty=-np.pi/Ly,startz=-np.pi/Lz,stepy=dky,stepz=dkz,yhat=yhat,zhat=zhat) for ik in np.arange(nk)])
# #XY=np.zeros((k_points_coords.shape[0],3))
# # for i in range(XY.shape[0]):
# #     XY[i,:] = k_points_coords[i,0]*yhat + k_points_coords[i,1]*zhat 
# ax.scatter(XY[:,0], XY[:,1])
# # ax.scatter(ik_list[:,1], ik_list[:,0])

# ax.scatter(K[0], K[1])
# ax.scatter(-K[0]+b_2[0], -K[1]+b_2[1])
# ax.scatter(-K[0]+b_3[0], -K[1]+b_3[1])
# ax.scatter(K[0]-b_2[0], K[1]-b_2[1])
# ax.scatter(K[0]-b_3[0], K[1]-b_3[1])
# ax.scatter(-K[0], -K[1])
# ax.scatter(Gamma[1], Gamma[2])
# ax.set(xlabel='entry a', ylabel='entry b')
# # plt.scatter(k_cut_list[:,0],k_cut_list[:,1])

# plt.scatter(k_points_coords[np.round(ind_final),:][:,0],k_points_coords[np.round(ind_final),:][:,1])
# ax.scatter(M[0],M[1])
# ax.scatter(-M[0],-M[1])
# plt.savefig(folder_save+'Plot-cut.pdf')
# #plt.show()
...
# ax.scatter(XY[:,0], XY[:,1])
# # ax.scatter(ik_list[:,1], ik_list[:,0])
# shape=np.array([K,b_3-K,K-b_2,-K,K-b_3,b_2-K,K])
...
# # ax.scatter(K[0]-b_3[0], K[1]-b_3[1])
# # ax.scatter(-K[0], -K[1])
# ax.scatter(Gamma[0], Gamma[1])


# ax.set(xlabel='entry a', ylabel='entry b')
# # plt.scatter(k_cut_list[:,0],k_cut_list[:,1])

# ax.plot(k_points_coords[np.round(ind_final),:][:,0],k_points_coords[np.round(ind_final),:][:,1], linestyle='none', markersize=10,marker='<',color='k')
# plt.arrow(k_cut_list[0][0],k_cut_list[0][1],k_cut_list[-1][0]-k_cut_list[0][0],k_cut_list[-1][1]-k_cut_list[0][1],length_includes_head=True, head_width=0.08, head_length=0.2,color='red')



# plt.savefig(folder_save+'plot-cut2.pdf')
# #plt.show()

# %%
nk_cut=len(k_cut_list)
cur_nn=np.zeros((nk_cut,nen,2),dtype='complex')
G_retarded_diag_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
G_lesser_diag_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
G_greater_diag_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
R_in_nn=np.zeros((nk_cut,nb*length,nb*length,nen),dtype='complex')
R_out_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
te_nn=np.zeros((nk_cut,nen,2,2),dtype='complex')

cur_balist_nn=np.zeros((nk_cut,nen,2),dtype='complex')
G_retarded_diag_balist_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
G_lesser_diag_balist_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
G_greater_diag_balist_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
R_in_balist_nn=np.zeros((nk_cut,nb*length,nb*length,nen),dtype='complex')
R_out_balist_nn=np.zeros((nk_cut,nb*length,nen),dtype='complex')
te_balist_nn=np.zeros((nk_cut,nen,2,2),dtype='complex')
...
for j in range(len(k_cut_list)):
    ...
    G_retarded_nn,G_lesser_nn,G_greater_nn,cur_nn[j],te_nn[j] = gf_dense.calc_gf(ne=nen,e=energies,num_lead=2,nm_dev=nb*length,nm_lead=dim_lead,max_nm_lead=nb*ns,
                                                    ham=ham[:,:,ik],lead_h00=lead_h00[:,:,:,ik],lead_h10=lead_h10[:,:,:,ik],
                                                    siglead=siglead,t=lead_coupling[:,:,:,ik],
                                                    scat_sig_retarded=sig_r[:,:,:,ik],scat_sig_lesser=sig_l[:,:,:,ik],scat_sig_greater=sig_g[:,:,:,ik],
                                                    mu=mu,temp=temp,flatband=False)
    ...
    G_retarded_balist_nn,G_lesser_balist_nn,G_greater_balist_nn,cur_balist_nn[j],te_balist_nn[j] = gf_dense.calc_gf(ne=nen,e=energies,num_lead=2,nm_dev=nb*length,nm_lead=dim_lead,max_nm_lead=nb*ns,
                                                    ham=ham[:,:,ik],lead_h00=lead_h00[:,:,:,ik],lead_h10=lead_h10[:,:,:,ik],
                                                    siglead=siglead,t=lead_coupling[:,:,:,ik],
...
for i in range(nb*ns):
    ax[0].plot(x,band_struct[:,ind_final][i],'--',color='white')

pcm = ax[0].pcolormesh(x, energies, A.T, vmin=0, vmax=10, cmap='hot')
fig.colorbar(pcm, ax=ax[0])
ax[0].set_title('Band structure obtained after a cut and used closest neighbour')
ax[0].set_xlabel('k points indices of the cut')
ax[0].set_ylabel('Energies')
ax[0].set_ylim(0, 20)

A = -np.sum(np.imag(G_retarded_diag_balist_nn), axis=1)

pcm = ax[1].pcolormesh(x, energies, A.T, vmin=0, vmax=10, cmap='hot')
fig.colorbar(pcm, ax=ax[1])
for i in range(nb*ns):
    ax[1].plot(x,band_struct[:,ind_final][i],'--',color='white')
ax[1].set_title('Band structure obtained after a cut and used closest neighbour')
ax[1].set_xlabel('k points indices of the cut')
...
pcm = ax[0].pcolormesh(x, energies, A.T, vmin=-10, vmax=4, cmap='hot')
fig.colorbar(pcm, ax=ax[0])
for i in range(nb*ns):
    ax[0].plot(x,band_struct[:,ind_final][i],'--',color='white')
# ax[0].set_title('Band structure obtained after a cut and used closest neighbour')
...
A = np.log(np.sum(np.imag(G_lesser_diag_balist_nn), axis=1))

pcm = ax[1].pcolormesh(x, energies, A.T, vmin=-15, vmax=4, cmap='hot')
fig.colorbar(pcm, ax=ax[1])
for i in range(nb*ns):
    ax[1].plot(x,band_struct[:,ind_final][i],'--',color='white')
# ax[1].set_title('Band structure obtained after a cut and used closest neighbour')
...
cmap = cm.seismic  # Colormap that goes from blue to red
norm = colors.Normalize(vmin=np.min(I2[:,mask][np.reshape(ind[:,0],-1)[:nb//2],:]), vmax=np.max(I2[:,mask][np.reshape(ind[:,0],-1)[:nb//2],:]))
...
#     I=0
#     for ik in range(nk):
#         I+=np.sum(ham[i,j,ik]*G_lesser[ik,i,j,:]-ham[j,i,ik]*G_lesser[ik,j,i,:])
